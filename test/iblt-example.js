...
console.log(
  'Did we successfully decode the substracted iblts?',
  result.success,
  result.reason
)
console.log(
  'Missing elements for iblt: ',
  result.missing,
  result.missing.map(e => e.toString())
)
console.log(
  'Additional elements of iblt and missing elements of the remote iblt: ',
  result.additional,
  result.additional.map(e => e.toString())
)
// create the iblt like before
console.log(
  'Verify if Buffer.from("help") is in the iblt: ',
  iblt.has(Buffer.from('help'))
)
...
console.log(
  'Deleting Buffer.from("help") and rechecking:',
  iblt.has(Buffer.from('help'))
)
